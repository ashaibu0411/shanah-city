...
export async function userIsInWorshipGroup(userId: string) {
  const configuredId = getConfiguredWorshipGroupId();
  const configuredGroup = await getGroupDetail(configuredId, userId);
  if (configuredGroup?.isMember) {
    return true;
  }
  ...
  for (const group of groups) {
    ...
    const groupDetail = await getGroupDetail(group.id, userId);
    if (groupDetail?.isMember) return true;
  }
  return false;
}
