...
export function TextSizeControl({ variant = "mobile" }: TextSizeControlProps) {
  ...
  const stepButtonClass = isMobile
    ? "flex h-10 w-10 items-center justify-center rounded-xl bg-white text-lg font-bold text-night-800 ring-1 ring-night-900/10 transition enabled:hover:bg-sand-50 disabled:cursor-not-allowed disabled:opacity-35 dark:bg-night-800 dark:text-sand-100 dark:ring-white/10 dark:enabled:hover:bg-night-700"
    : "flex h-9 w-9 items-center justify-center rounded-xl border border-night-900/10 bg-white text-base font-bold text-night-800 transition enabled:hover:bg-sand-100 disabled:cursor-not-allowed disabled:opacity-35 dark:border-white/10 dark:bg-night-800 dark:text-sand-100 dark:enabled:hover:bg-night-700";

  return (
    <div className={isMobile ? "" : "flex items-center gap-3"}>
      {!isMobile && (
        <div className="hidden min-w-[7.5rem] lg:block">
          <p className="text-xs font-semibold text-night-900 dark:text-sand-100">Reading size</p>
          <p className="text-[11px] text-night-500 dark:text-sand-400">{currentLabel}</p>
        </div>
      )}

      {isMobile && (
        <>
          <p className="text-sm font-semibold text-night-900 dark:text-sand-100">Reading size</p>
      ...
      <div className={isMobile ? "mt-3 space-y-3" : "flex items-center gap-2"}>
        ...
        <div
          className={
            isMobile
              ? "grid grid-cols-4 gap-1 rounded-xl bg-sand-100/90 p-1 ring-1 ring-night-900/8 dark:bg-night-800/80 dark:ring-white/10"
              : "hidden"
          }
          role="group"
