...
import { UrgentAlertDetailDialog } from "@/components/urgent-alert/UrgentAlertDetailDialog";
...
export function UrgentAlertCarousel({
  ...
  useEffect(() => {
    if (highlightAlertId && alerts.some((alert) => alert.id === highlightAlertId)) {
      setDetailId(highlightAlertId);
    }
  ...
  const detailAlert = detailId ? alerts.find((entry) => entry.id === detailId) ?? null : null;
    ...
    <>
      ...
      >
        ...
        <div className="relative">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-red-100">
                {count > 1 ? `Announcement ${index + 1} of ${count}` : "Urgent alert"}
              </p>
            </div>
            {count > 1 ? (
          ...
          {count > 1 ? (
            <div className="mt-4 flex justify-center gap-1.5" role="tablist" aria-label="Choose announcement">
              {alerts.map((entry, dotIndex) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`Show announcement ${dotIndex + 1}: ${entry.title}`}
      ...
      <UrgentAlertDetailDialog
        alert={detailAlert}
        open={Boolean(detailId && detailAlert)}
